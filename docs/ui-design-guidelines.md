# PaNasMs interface design standard

Version 2.0 · 2026-09-23 · Pavlo's NAS Management System
...
This is the single canonical design standard for core and module UI contributions.
It consolidates the earlier UI/UX and modal-dialog guidelines; their old URLs
remain reference pages, not independent specifications.
...
1. Identify the user's task, role, target object and owning module. Choose a page,
   popover, dialog or background task using NAV-02 and MOD-01.
2. Sketch its hierarchy, navigation and action placement before styling. Use the
   pattern table below; do not create a new visual pattern for each module.
3. Specify loaded, empty, loading, error, stale and denied states, plus selection,
   unsaved edits and waiting/recovery where relevant. State what closing means.
4. Reuse the shared component, semantic tokens, icons and translations. If a
   pattern is missing, extend the shared contract rather than forking it locally.
5. Validate the changed workflow against the final checklist. Record gaps and
   exceptions by rule ID; do not equate a successful build with UX acceptance.

| Screen or control | Required pattern |
| --- | --- |
| Top-level section tabs | Full-height joined strip and content; VIS-06 |
| Tabs inside a page or dialog | Flat underline navigation; VIS-06, MOD-09 |
| Settings and personal profile | Left navigation, vertical divider, flat right content; FORM-07 |
| Object collection | Compact consistent cards or an appropriate hierarchy/table; CMP-05, CMP-11 |
| Module details / Cloud Sync details | One coherent content page, no nested shadow-card stack; NAS-08/09 |
| Existing folder, user or group | Shared visual/searchable selector; FORM-02/06 |
| One bounded edit or inspection | Shared dialog with explicit regions; MOD-01–12 |
| Routine result | Shared toast; active problems also remain with their object; OPS-04 |
| Long operation | Background task, compact taskbar indicator and useful details; OPS-03 |

Do not silently weaken an accepted requirement to match unfinished code. Current
explicit owner decisions supersede older examples; update this standard when a
new pattern is accepted. Research is supporting evidence, not permission to undo
product decisions. Compliance and implementation progress are assessed separately.
...
Top-level horizontal section tabs and their content form one panel. The tab strip has a subtle
...
Inside pages, settings subsections and dialogs, use flat underline tabs. Do not
repeat the full-height joined strip there. Settings/profile category navigation
uses FORM-07 instead. Use link semantics for route navigation and tab semantics
with the corresponding keyboard behavior for an in-place tab panel.
...
**CMP-07 — Dialogs. MUST.** Follow [MOD-01–12](#10-modal-dialogs) for
surface selection, explicit layout, the shared internal close icon, dirty-state
policy, focus, scrolling and background-task handoff. Core and modules use the
same contract. A dialog wrapper alone does not establish conformance.

**CMP-08 — Confirmations. MUST.** Confirm destructive or disruptive intent once,
with the exact object, practical consequence and normally two clear decisions. Use a short
...
**CMP-11 — Tables belong to the page. MUST.** Use one flat table/list surface
with clear column headings, aligned values, light row separators and subtle
hover/selection. Do not wrap every table in another bordered, rounded, shadowed
"page". Actions stay compact and aligned; sorting/filtering states remain clear.
Keep a semantic table for comparable records, with accessible sort controls.
Contain horizontal overflow only when the data requires two-dimensional reading;
otherwise wrap cells or use a labeled narrow-screen list. Empty, loading and
failed requests remain distinct. Dense data is not a reason to reduce essential
text below the shared label scale.
...
**FORM-07 — Settings and profile layout. MUST.** Use category navigation on the
left, one vertical divider and a flat content region on the right. Do not enclose
the layout in an extra frame or split ordinary form sections into shadow cards.
Separate related sections with spacing and single dividers; avoid doubled lines.
On narrow screens, collapse navigation into an accessible category selector or
navigation row before narrowing the form. Global Settings and personal Profile
use the same geometry but keep their different ownership. Use `general` and
`advanced` route segments for their corresponding settings levels. General NAS
settings include the web port and CPU cooling; disk cooling stays with Storage.
...
**Toast close control. MUST.** Use the shared close icon over the top-right corner,
centered on the corner so half the visible icon extends above and to the right.
Keep its larger accessible hit area, focus ring and surrounding clearance unclipped.
No text X, heavy square button or close control in the notification content row.
Follow [MOD-03](#mod-03).
...
**NAS-09 — Module workspaces. MUST.** A module details page is one coherent page,
not several nested shadow cards. Cloud Sync uses a left list of configured syncs
and a flat right details area with the folder pair and history together. Folder
paths use the same friendly location/array identity as Files, followed by the
relative path. Its setup combines related folder/direction choices rather than
splitting each field into its own wizard step.

Terminal opens directly into its workspace, with internal tabs and a compact plus
action for another session. Connection state is a small named icon at the terminal's
upper right. Navigation preserves existing sessions/history; active sessions remain
visible through the taskbar with a supported stop action. Never imply a remote
terminal stopped simply because its page is hidden.
...
### MOD-01 — Choose the right surface

A modal MUST have one bounded purpose. Select the surface before designing it:

| Task | PaNasMs surface |
| --- | --- |
| Rename, create a folder, confirm eject | Compact dialog |
| Edit one share or access rule | Form dialog |
| Inspect one disk's SMART data or one network interface | Details dialog |
| Configure network sharing through a short sequence | Wizard in one dialog |
| Browse modules, manage users, use Files or Terminal | Routed page |
| View notifications, tasks or active terminal sessions | Top-bar popover |
| File transfer, RAID maintenance, accepted update job | Background task with progress and a route back |
| Save succeeded or a routine operation failed | Toast; persistent task/notification record when needed |
| Field validation failed | Inline error in the existing form |

Frequent navigation, independent sub-workflows, or substantial configuration
requiring several unrelated decisions SHOULD become a page. A bounded SMART table
is not automatically disqualified because it scrolls. Do not turn every module
into a dialog merely because a reusable wrapper makes it possible.

No new generic error/success/loading modal. A user-requested blocker report may
use the existing operation dialog: identify the process/service/path blocking
unmount or home migration, and offer an actionable next step. Errors must not
silently discard input.

<a id="mod-02"></a>

### MOD-02 — One coherent surface

A form, wizard or substantial details window MUST have explicit **header, body
and optional action footer**. Use one background, not cards inside a card.

- Header: specific title, optional object metadata, optional compact toolbar.
- Body: fields, table, results or decision text. Group with spacing and thin lines.
- Footer: final decisions for the entire dialog, when there are any.
- Inspection dialogs need no empty footer or redundant OK button.
- A compact confirmation may omit visual section dividers; it must not reserve
  a form-sized empty body or a tall footer.

Titles name the task and target, e.g. `Network details · wlan0`. Avoid “Management”,
“Operation plan” or repeated subtitles. Display long names without clipping the
only identification of the target; permit wrapping rather than shrinking text.

<a id="mod-03"></a>

### MOD-03 — Consistent close icon

Every modal MUST use the same shared close icon **inside the header at the top
right**, on desktop and narrow screens. Do not mix a text `X` or `×`, an icon, and a visibly boxed square button.

| Property | Rule |
| --- | --- |
| Icon | Existing MDI close glyph, 20 CSS px, same stroke and size everywhere |
| Hit area | 44 × 44 CSS px; transparent at rest, no permanent border or square button background |
| Placement | Inside the header at its right edge, using shared header padding/alignment |
| Interaction | Subtle rounded hover/pressed treatment and a clear keyboard focus ring |
| Tooltip and accessible name | Localized “Close”; tooltip also available on keyboard focus |

Use a semantic button despite the unboxed appearance. Reserve header space so the
title never overlaps it. Keep the close control visible while the body scrolls and
inside the dialog's DOM/focus scope. Its appearance, alignment and behavior are
shared by core and modules; no per-window variants.
[WAI-ARIA dialog pattern](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/).

**Toast exception:** use the same close glyph, overlaid at the toast's top-right
corner. Center the visible icon on that corner, so half its width protrudes right
and half its height protrudes above the toast (10px each for the 20px glyph).
This offset is based on the visible icon, not half the larger invisible hit area.
Do not put it in the notification's text/action row or render a heavy square button.
Retain the accessible hit area and focus ring; reserve stack/viewport clearance
for them and do not clip them with overflow. The icon must remain legible over
both the toast and the surrounding wallpaper, using a subtle theme-aware backing
if needed. Dismissing a toast does not acknowledge or resolve its underlying alert.

<a id="mod-04"></a>

### MOD-04 — Size and position

Use named sizes, not a new width for every call site:

| Variant | Preferred panel width | Placement |
| --- | --- | --- |
| Confirmation / single-field input | Up to 440px; may be smaller | Centered while comfortably fitting |
| Form / folder picker / short wizard | 640px | Top anchored, normally 40px below viewport top |
| Details / SMART / bounded table | 800px | Top anchored, normally 40px below viewport top |
| Exceptional dense details | Up to 960px, with documented need | Same anchoring; otherwise use a page |

Panel widths MUST clamp to available space; no external-close gutter is needed.
Forms/details have no arbitrary minimum height. Use `dvh`-aware maximum height,
16px minimum vertical clearance and safe-area insets. On short viewports reduce
the top gap first. At 320px width use 16px side clearance and an internal close.

Keep width and top position stable through validation, searching and wizard steps.
For tabbed inspection such as SMART, preserve the dialog frame height when changing
tabs while the viewport stays unchanged; do not make its header jump between the
screen center and top. Scrolling belongs to each tab's body, with its position
restored on return. A viewport/orientation change may recompute the frame.

No universal 25%-of-screen limit, 400px minimum, 80px width multiple or 96px footer.

<a id="mod-05"></a>

### MOD-05 — Scrolling and visual tokens

The header, footer and close icon MUST stay reachable while the **body**
scrolls. Lock the background page and restore its exact scroll position on close.
Use one vertical body scroller; avoid an independently scrolling form inside it.
A folder tree or long option list may scroll separately when needed for selection.

Reserve scrollbar space to avoid horizontal shifts. A boundary/shadow at the
scrolling edge may signal overflow but must not obscure rows. Do not force a fake
scrollbar when no overflow exists. A wide technical table may have a contained
horizontal scroller; ordinary fields, explanations and the dialog shell may not.
The application body must not move when a scrollbar appears.

Initial shared tokens: 24px desktop section padding, 16px narrow-screen padding;
8px label/control gaps; 16px field gaps; 24px section gaps; 1px dividers; existing
16px dialog radius and raised shadow. Footer height follows its content.
Section titles use the existing 20/28px scale, body 16/24px, secondary metadata
14/20px. Compact data tables may use the project's existing table scale.

Use a single dark neutral backdrop, initially `rgba(15, 23, 42, 0.48)`, adjustable
only through a shared theme token. Blur is not required. The panel is opaque in
both themes; wallpaper must not reduce its readability. Normal text contrast is
at least 4.5:1; required icon/control and focus indications at least 3:1 against
adjacent colors. Validate actual rendered themes, not token names.
[Text contrast](https://www.w3.org/WAI/WCAG22/Understanding/contrast-minimum.html),
[non-text contrast](https://www.w3.org/WAI/WCAG22/Understanding/non-text-contrast.html).

<a id="mod-06"></a>

### MOD-06 — Actions and wording

Object toolbars use compact icons with tooltip and accessible name, as requested
for SMART. Place metadata left and toolbar right under the title; wrap sensibly
on small screens. Do not confuse toolbar operations with committing the form.

Final decisions MUST have visible words: Save, Apply, Delete, Cancel. The project's
preference for icons does not replace these decision labels with unexplained icons.
Use at most one visually primary decision. In our left-to-right languages,
secondary/cancel precedes the primary action on the right. Keep DOM and visual
order aligned; wrapping must not change their logical order.

Confirmations identify the target in bold and explain the practical consequence.
Never require typing a file path, object name or DELETE. Keep an already agreed
Yes/No prompt if the question is unambiguous. Do not add a second confirmation for
the same decision.

Normally use two decisions, but preserve explicit three-way workflows:

- File removal: **Move to Trash / Delete / Cancel**.
- Damaged removable filesystem: **Repair / Read-only / Cancel**.
- Unsaved edits: **Keep editing / Discard**; Save only if it can safely complete.

Destructive choices use danger styling and never receive initial focus. No generic
“Continue” when the click will format, delete or disconnect something immediately.

<a id="mod-07"></a>

### MOD-07 — Closing and dirty state

Close is not Save, and closing a progress view is not Cancel operation. All dismissal
paths use one policy. The user must have an explicit exit; no permanently locked
modal for authentication or an offline NAS.

| State | Close icon / Cancel / Escape | Backdrop |
| --- | --- | --- |
| Read-only details | Close, restore caller context | Close |
| Pristine form | Close without writing | Close |
| Form with unsaved changes | Ask Keep editing / Discard in the same shell | Ignore to avoid accidental loss |
| Destructive decision awaiting confirmation | Dismiss without executing | Ignore |
| Picker/secondary wizard step | Close requests exit of the flow; Back returns one step | Follow form's dirty policy |
| Accepted background job | Close view; job continues | Close view |
| Submission pending, acceptance unknown | Show honest pending/reconciliation state; see MOD-10 | Ignore |

A drag that starts in the panel and ends outside MUST NOT close it. Backdrop dismissal
requires the gesture to begin and end on the backdrop. Dropdowns, calendar popovers,
folder-tree interaction and the close control are not backdrop clicks.

Escape first dismisses an open child popup; a subsequent Escape dismisses the dialog
or requests the dirty-state decision. It must never run the primary action.
Navigation/Back uses the same dirty guard. Opening inspection must not execute writes;
Back/Forward must never replay a submission.

<a id="mod-08"></a>

### MOD-08 — Keyboard and assistive technology

Use dialog semantics, an accessible title and `aria-modal`. Trap Tab/Shift+Tab;
make background controls inert. Return focus to the trigger or a logical successor
if that object was removed. Initial focus: first relevant field for a simple form,
title for structured details, Cancel for destructive confirmation. Avoid describing
a whole table via `aria-describedby`.
[WAI-ARIA dialog guidance](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/).

Use `alertdialog` only for a brief consequential decision, with its warning
associated accessibly. It is not the role for every form or SMART viewer.
[WAI-ARIA alert dialogs](https://www.w3.org/WAI/ARIA/apg/patterns/alertdialog/).

Use the shared 40px control target, 44px for close/touch use; reserve the
32px dense-row exception for mouse-oriented table actions, never final decisions. WCAG 2.2 AA's
minimum pointer-target criterion is 24px with defined exceptions; that is not our
preferred size. The larger project target does not establish full compliance.
[Target size](https://www.w3.org/WAI/WCAG22/Understanding/target-size-minimum.html).

At 200% text sizing and 400% page zoom, essential controls and explanations remain
reachable. Test reflow at 320 CSS px, including long names and translated buttons.
[Reflow](https://www.w3.org/WAI/WCAG22/Understanding/reflow.html).

No blanket Enter listener on the dialog. Respect text areas, autocomplete and IME;
Enter on the focused action works normally. Announce errors/status changes without
repeatedly reading a timer or moving focus on metric updates. Respect reduced
motion; use only a short shared fade (initial target 120ms), no moving/scaling window.

<a id="mod-09"></a>

### MOD-09 — Tabs, pickers and workflows

Inside dialogs use our **internal tabs**: a simple row with an underline and an
active indicator. Full-height joined tabs belong to top-level pages. Do not add a
separate rounded button background to every tab. Selection and keyboard focus are
distinct; retain input and per-tab scroll state.

One modal shell and one backdrop per workflow. An action inside SMART changes the
body to a short confirmation/review state with Back; it must not open another full
modal on top. Folder selection in a share form uses an inline tree or a picker step
inside that same shell, retaining the parent form. The same applies to dirty-state
confirmation. Small list/select/tooltips are popovers within the active dialog's
layer, not another modal window.

Wizard steps retain data and stable width. Show progress through steps only when it
helps the user understand what remains; do not manufacture a multi-step flow for a
single field. Full module management stays on its routed page.

<a id="mod-10"></a>

### MOD-10 — Waiting, background work and errors

Preserve the owner's shared waiting pattern: when the current dialog cannot accept
input during an operation, cover it with a translucent layer, spinner and specific
message. Disable conflicting actions, including the close control while acceptance
is unresolved. The layer is inside the same dialog, not another modal.

This is a **temporary submission state**, not permission for an endless lock.
On request deadline or lost connection, replace the spinner with a recoverable
status: retain the operation ID and input, reconcile with the server, allow closing
the view, and do not blindly repeat a possibly accepted destructive command.
Closing must not claim cancellation. No automatic timeout is allowed to roll back
an operation unless the backend actually provides that behavior.

Once the backend has accepted a long task, expose it through Tasks and the taskbar
and allow the modal to close. File uploads/copies/moves/deletions and RAID work
MUST NOT require keeping a dialog open. Cancel is offered only when supported and
means a real cancellation request, with the backend's final result shown.

For form errors, preserve input and show the issue next to the relevant field;
focus the first invalid field or a concise error summary. Operational blockers
name the cause and remedy, not only a PID or generic failure. Successful completion
uses the shared toast mechanism, not a long-lived success block or another dialog.

<a id="mod-11"></a>

### MOD-11 — System-initiated decisions

Ordinary monitoring events use notifications without stealing focus. Preserve the
previously agreed removable-media flow as an explicit exception: detection may
request Repair / Skip; attempting to mount damaged media offers Repair / Read-only /
Cancel. Queue that prompt behind a current dialog and coalesce it by device/state;
never repeatedly interrupt after repair or overwrite unsaved work.

Network rollback confirmation remains a focused dialog with a real deadline and
Keep / Revert actions. Closing cannot mean Keep implicitly. State clearly whether
closing requests reversion or leaves the countdown running, matching the backend
contract. Do not treat a hidden window as a security boundary: all authorization
and validation remain server-side.

<a id="mod-12"></a>

### MOD-12 — Shared implementation contract

Implement in the shared React/Radix layer and expose through the module SDK.
[Radix Dialog](https://www.radix-ui.com/primitives/docs/components/dialog) supplies
focus/dismiss hooks and dialog primitives; it does not choose our dirty-state,
background-task or layout policies. Reuse its primitives rather than implementing
another focus trap in each module.

Target explicit slots: shell, panel, header, metadata/actions, body, footer, close,
waiting layer. The shared [`DialogContent`](https://github.com/PaNasMs/frontend/blob/main/src/shared/waiting.tsx) accepts explicit
header/footer slots. Child classification remains only as a compatibility adapter
for previously released modules. New and migrated call sites use the explicit API.

The shared owner controls size, placement, backdrop, close-control geometry,
scroll lock, dismissal reason and focus restoration. Call sites provide content,
actions, initial-focus intent, dirty state and operation state. Modules must not
fork these policies or change global dialog CSS. Preserve existing server previews,
permissions and operation fingerprints; a visual simplification removes no checks.

Explicit states: viewing/editing, dirty-close decision, submitting, accepted job,
validation error, operational failure, disconnected/unknown outcome. Test their
transitions, not only screenshots. Existing implementations are not automatically
compliant because they import `DialogContent`.

## 11. Research basis and deliberate adaptations

General sources were consulted on **2026-09-21**; modal sources and the supplied
article discussions were reviewed on **2026-09-23**. W3C defines accessibility criteria;
...
The [modal research notes](../documentation/modal-dialog-guidelines.md#research-notes)
retain the assessment of Habr, its comments, Medium and the additional references.
Useful conclusions are stable geometry, reachable actions, restrained interruption
and body scrolling. Universal 25%-of-screen limits, two-button maxima, fixed footer
heights and no-scroll rules were rejected. WAI-ARIA dialog/alertdialog patterns,
Radix primitives and Carbon modal guidance inform MOD-01–12; none replaces our
accepted choices or proves the current implementation compliant.
...
- Verify shared tokens, top-level joined tabs versus internal underline tabs,
  flat settings/profile layout and restrained elevation.
- Record deviations and untested cases explicitly before claiming conformance.

### Dialog and toast acceptance

For every migrated dialog record variant, trigger, role, language, theme, viewport,
operation state and the result of each applicable check. **Not tested** is not Pass.

- [ ] Correct surface and bounded purpose; no page-sized management hub in a modal.
- [ ] Header/body/footer agree with the variant; no decorative nested cards.
- [ ] One uniform close icon inside the modal header at every size; no text X or boxed variants.
- [ ] Toast close icon overlaps the top-right corner by half its visible size; hit area/focus ring unclipped.
- [ ] Close, focus ring and tooltip work against light/dark/busy wallpaper.
- [ ] Stable frame during errors, loading and tab changes; no scrollbar layout jump.
- [ ] Background locked; header/actions reachable; no accidental nested scroll trap.
- [ ] Keyboard-only open, traversal, child-popup Escape, dialog Escape and focus return.
- [ ] Screen-reader name/description and safe initial focus; close control is in scope.
- [ ] Clean/dirty/backdrop/drag-out/Back cases follow MOD-07; no silent input loss.
- [ ] 320, 390, 768, 1024 and 1440 CSS px; short landscape viewport; 200% text and 400% zoom.
- [ ] EN/RU/UK, long filename/path and enlarged text; mobile keyboard does not cover actions.
- [ ] Short wait, rejected request, disconnected/unknown outcome and accepted background job.
- [ ] Closing a task view does not cancel work; cancellation reports its real outcome.
